 ...
 *
 *     //pass one doc
 *     Candy.create({ type: 'jelly bean' }, function (err, jellybean) {
 *       if (err) // ...
 *     });
 *
 *     //pass individual docs
 *     Candy.create({ type: 'jelly bean' }, { type: 'snickers' }, function (errors, docs) {
 *       if (errors) // ...
 *     });
 *
 *     //pass an array
 *     var array = [{ type: 'jelly bean' }, { type: 'snickers' }];
 *     Candy.create(array, function (errors, docs) {
 *       if (errors) // ...
 *     });
 *
...
Model.create = function create (doc, fn) {
  ...
  args.forEach(function (arg, i) {
    ...
    docs[i+1] = doc;
    doc.save(function (err) {
      if (err) errors.push(err); else savedDocs.push(this);
      if (!--count) return fn(multipleDocs ? errors : errors[0], multipleDocs ? savedDocs : savedDocs[0])
    });
  });
};
